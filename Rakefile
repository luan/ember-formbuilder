...
def compile_package_task(input, output=input)
  ...
  `mkdir -p tmp/packages/#{input}/lib/`
  `cp -rf packages/#{input}/lib/ tmp/packages/#{input}/lib/`
  `find tmp/packages/#{input}/lib -iname *.coffee -exec coffee -c {} +`
  `find tmp/packages/#{input}/lib -iname *.coffee -delete`

  js_tasks = SproutCore::Compiler::Preprocessors::JavaScriptTask.with_input "tmp/packages/#{input}/lib/**/*.js", "."
...
task :build => ["ember:formbuilder"]

distributions = {
  "ember-formbuilder" => ["ember-formbuilder"]
}
...
desc "Upload latest Ember FormBuilder build to GitHub repository"
task :upload => :dist do
  # setup
  ...
  puts "Uploading ember-formbuilder-latest.js"
  gh.replace(
    :repos => repo,
    :file  => 'dist/ember-formbuilder.js',
    :name => 'ember-formbuilder-latest.js',
    :content_type => 'application/json',
    :description => "Ember FormBuilder Master"
  )

  puts "Uploading ember-formbuilder-latest.min.js"
  gh.replace(
    :repos => repo,
    :file  => 'dist/ember-formbuilder.min.js',
    :name => 'ember-formbuilder-latest.min.js',
    :content_type => 'application/json',
    :description => "Ember.js FormBuilder Master (minified)"
  )
end
