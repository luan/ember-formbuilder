...
module "Ember.FormBuilder.Form"
  setup: ->
    form = Ember.FormBuilder.Form.create()

  teardown: ->
    form = null

test "can have a property set on it", ->
  set form, 'name', 'bar'
  equal get(form, 'name'), 'bar', "property was set on the form"

test "generates a form tag", ->
  ok form.tagName == 'form', "should have inserted a form tag."

test "displays children when template has them", ->
  set form, 'template', Handlebars.compile "<input type=\"text\" /><input type=\"text\" />"
  appendView form
  ok form.$().find('input').length == 2, "should have children."

test "has the class names specified", ->
  classes = 'form-horizontal class1 class2'
  set form, 'classes', classes
  appendView form

  for cls in classes.split(' ')
    ok form.$().hasClass(cls), "form should have class #{cls}"
